...
export const createClient = (): SupabaseClient<Database> => {
  ...
  try {
    ...
    connectionAttempts++;
    console.error(`Error creating Supabase client (attempt ${connectionAttempts}):`, error);
    ...
    connectionAttempts = 0;
    throw new Error('Authentication service unavailable after multiple attempts');
  }
}
...
export async function testSupabaseConnection() {
  ...
  while (retries < MAX_RETRIES) {
    try {
      ...
      const { data, error } = await supabase.from('_status').select('time').limit(1);
